...
echo "4. Verifying KFP SDK..."
KFP_VERSION=$(podman run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 -c "import kfp; print(f'kfp {kfp.__version__}')")
echo "   ✅ ${KFP_VERSION}"
echo ""

echo "5. Verifying linting tools..."
BLACK_CHECK=$(podman run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 -c "import black; print('black installed')")
FLAKE8_CHECK=$(podman run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 -c "import flake8; print('flake8 installed')")
PYLINT_CHECK=$(podman run --rm "${IMAGE_NAME}:${IMAGE_TAG}" python3 -c "import pylint; print('pylint installed')")
echo "   ✅ ${BLACK_CHECK}"
echo "   ✅ ${FLAKE8_CHECK}"
echo "   ✅ ${PYLINT_CHECK}"
echo ""

echo "6. Verifying ML libraries..."
...
echo "7. Verifying infrastructure clients..."
...
echo "====================================="
echo ""
echo "Image is ready to use in Helm chart:"
